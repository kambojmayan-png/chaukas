...
<!-- Screenshots are generated by `npm run screenshots` (Playwright, 390x844, Hindi) into docs/screens/. -->
...
<p align="center"><img src="docs/screens/05-check.png" width="220" alt="Message checker showing a spoken verdict and highlighted pressure tactics"></p>

## Three rounds of outside feedback, in one day
...
**5 pm: "it's cut off on the right on my phone."** The third phone-layout bug of the day, and the third one found by a person holding a phone. So it became a test: `npm run test:layout` drives a real browser at 320, 360 and 412 px wide through the whole first practice, from the home screen to the lesson, and on every screen asserts that nothing is wider than the phone, that the main button is on screen, and that "hear again" is present. Its first run caught the cause at once (a top bar that could not wrap made every page 100px too wide); it now passes at all three sizes. The same round gave the English guide a female voice, kept the callers male, and put "hear again" on the screens that lacked it.
...
<p align="center"><img src="docs/screens/06-insights.png" width="220" alt="Live insights page with the Knowledge-Behaviour Gap and its sample size"></p>
...
| Technical implementation | `src/engine/engine.ts` (pure and deterministic) · scenario linter and bot players in `tests/` · an automated phone-layout test (`tests/layout.spec.ts`) · the CI badge · the engine log in `/drill` · two interfaces on one engine |
...
**Stack:** Next.js (App Router) · TypeScript · Tailwind CSS · Supabase (Postgres) · Vercel · GitHub Actions · Playwright.
...
npm run build
npm run test:layout   # real-browser walk-through at three phone widths (first time: npx playwright install chromium)
...
tests/run-tests.mjs       bot players, graph validation, checker fixtures
tests/layout.spec.ts      phone-layout walk-through at 320 / 360 / 412 px
scripts/                  voice manifest + screenshot generators
...
| **16:56 – 17:13** | **After "it's cut off on my phone": overflow fixed, "hear again" everywhere, a female English guide voice, and an automated phone-layout test that now guards every push** |
